...
#define TAG "ti.crashlytics"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Crashlytics_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Crashlytics.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::CrashlyticsBindings::lookupGeneratedInit(
...
static void Crashlytics_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Crashlytics_getBinding)->GetFunction());
}

static void Crashlytics_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::CrashlyticsBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry CrashlyticsBinding = {
	"ti.crashlytics",
	Crashlytics_init,
	Crashlytics_dispose
};
...
Java_ti_crashlytics_CrashlyticsBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.crashlytics", &CrashlyticsBinding);
	titanium::KrollBindings::addExternalLookup(&(::CrashlyticsBindings::lookupGeneratedInit));
}
